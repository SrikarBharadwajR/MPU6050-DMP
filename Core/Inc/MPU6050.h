...
#define round_long(x) ((x) >= 0 ? (long)((x) + 0.5) : (long)((x) - 0.5))
...
bool writeMemoryBlock(
    const uint8_t* data, uint16_t dataSize, uint8_t bank, uint8_t address, bool verify, bool useProgMem);

// bool writeProgMemoryBlock(
//     const uint8_t* data, uint16_t dataSize, uint8_t bank = 0, uint8_t address = 0, bool verify = true);
bool writeProgMemoryBlock(const uint8_t* data, uint16_t dataSize, uint8_t bank, uint8_t address, bool verify);
bool writeDMPConfigurationSet(const uint8_t* data, uint16_t dataSize, bool useProgMem);    // useProgMem was false
...
void CalibrateGyro(
    uint8_t Loops);    // Default was 15                         // Fine tune after setting offsets with less Loops.
void CalibrateAccel(uint8_t Loops);    // Default was 15                                  // Fine tune after setting
                                       // offsets with less Loops.
void PID(uint8_t ReadAddress, float kP, float kI, uint8_t Loops);    // Does the math
void PrintActiveOffsets(void);                                       // See the results of the Calibration
